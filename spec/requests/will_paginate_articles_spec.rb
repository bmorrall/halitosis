  ...
  describe "GET /will_paginate_articles" do
    context "when on the first page" do
      it "returns the first 10 articles with pagination meta and links", :aggregate_failures do
        ...
        expect(body.dig("_links", "first", "href")).to eq(will_paginate_articles_path(page: {number: 1}))
        expect(body.dig("_links", "last", "href")).to eq(will_paginate_articles_path(page: {number: 2}))
        expect(body.dig("_links", "prev")).to be_nil
        expect(body.dig("_links", "next", "href")).to eq(will_paginate_articles_path(page: {number: 2}))
      end
    end
    ...
    context "when on the second page" do
      it "returns the remaining 5 articles with pagination meta and links", :aggregate_failures do
        ...
        expect(body.dig("_links", "first", "href")).to eq(will_paginate_articles_path(page: {number: 1}))
        expect(body.dig("_links", "last", "href")).to eq(will_paginate_articles_path(page: {number: 2}))
        expect(body.dig("_links", "prev", "href")).to eq(will_paginate_articles_path(page: {number: 1}))
